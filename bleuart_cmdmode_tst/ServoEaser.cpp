...
void ServoEaser::begin(Adafruit_PWMServoDriver s, int frameTime, 
...
{
    movesCount = 90;  // no longer doing moves list
    startPos = currPos;
...
void ServoEaser::update()
{
    if( ((millis() - lastMillis) < frameMillis) || !running ) {  // time yet?
      Serial.println("Jeremy not time yet");
        return;
    }
    lastMillis = millis();
    ...
    Serial.println("EASE TO A POSITION");
    servo.setPWM(pin, 0, angleToPulseLength(p));
}


void ServoEaser::setPin(int pin)
{
  pin = pin;
}
...
int ServoEaser::angleToPulseLength(float angle)
{
...
{
  Serial.print("Has Arrived"); Serial.print(arrived); Serial.println("");
    return arrived;
}
