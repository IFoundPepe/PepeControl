...
private:
    ...
    int angleToPulseLength(float angle);
...
public:
    ...
    void begin(Adafruit_PWMServoDriver s, int frameTime); //, int startPos);
    void begin(Adafruit_PWMServoDriver s, int frameTime, ServoMove* moves, int movesCount);
    ...
    // Set the pin number for the servo easer
    void setPin(int pin);
